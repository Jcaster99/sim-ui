  ...
  return (
    <Card sx={{ backgroundColor: "transparent", border: "none" }}>
      ...
      >
        {lqtyStake.ves.map(ve => (
          <div style={{ flexDirection: "column", margin: "0 20px" }} key={ve.tokenId}>
            <StaticRow label="TokenId" amount={ve.tokenId.toString()} />
            <StaticRow label="Locked" amount={ve.locked.prettify()} unit={GT} />
            <StaticRow
              label="Lock end"
              amount={new Date(+ve.lockEnd.toString() * 1000).toLocaleDateString("en-US")}
            />
            <StaticRow label="Redemption gain" amount={ve.earnedWSTETH.prettify()} unit="wstETH" />
            ...
            <StaticRow label="Issuance gain" amount={ve.earnedSIM.prettify()} unit={COIN} />
            {ve.earnedSIM.gt(0) && (
              <>
            ...
            <Button
              variant="outline"
              onClick={() => dispatch({ type: "startAdjusting", tokenId: ve.tokenId })}
            >
      ...
      <Flex variant="layout.actions">
        <Button
          variant="outline"
          onClick={() => dispatch({ type: "startAdjusting", tokenId: undefined })}
        >
          &nbsp;Create new lock
        </Button>
      </Flex>
